...
public class MenuAnimator : MonoBehaviour
{
    ...
    void StartSequence()
    {
        ...
        logoStart.Append(fadeImage.transform.DOScale(new Vector3(40, 40, 40), 1));
        logoStart.Append(logo.transform.DOMove(new Vector3(1000, 8000, 0), 0.5f).From());
        logoStart.Insert(1.25f, logo.transform.DOScale(new Vector3(2f, 0.2f, 1), 0.25f));
        logoStart.Append(logo.transform.DOScale(new Vector3(1, 1, 1), 0.25f));

        logoStart.Insert(1, logoImage.DOFade(1, 0.3f));
     
        logoStart.Append(logo.transform.DOScale(1.1f, 2).SetEase(Ease.InOutSine).SetLoops(-1, LoopType.Yoyo));

        logoStart.Insert(4f, playText.DOFade(1, 0.5f));
        logoStart.Insert(4f, optionsText.DOFade(1, 0.5f));
        logoStart.Insert(4f, quitText.DOFade(1, 0.5f));
